...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3,num_joint=17):
        ...
        self.Feature_Mask = nn.Parameter(torch.ones(1,self.num_joint,in_channels, requires_grad=True,device='cuda'),requires_grad=True)
        nn.init.constant(self.Feature_Mask, 0)

        self.bn = nn.BatchNorm1d(self.num_joint*out_channels)
        self.relu = nn.ReLU()
        ...
        index_array = np.empty(self.num_joint*in_channels).astype(np.int)
        for i in range(self.num_joint):
            for j in range(in_channels):
                index_array[i*in_channels + j] = (i*in_channels + j + j*in_channels)%(in_channels*self.num_joint)
        self.shift_in = nn.Parameter(torch.from_numpy(index_array),requires_grad=False)

        index_array = np.empty(self.num_joint*out_channels).astype(np.int)
        for i in range(self.num_joint):
            for j in range(out_channels):
                index_array[i*out_channels + j] = (i*out_channels + j - j*out_channels)%(out_channels*self.num_joint)
